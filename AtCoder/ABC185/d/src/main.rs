fn main() {
    ...
    let mut a: Vec<u64> = rd.get_vec(m);

    a.sort();
    let a = [vec![0], a, vec![n + 1]].concat();
    let dists: Vec<u64> = a.windows(2).map(|w| w[1] - w[0] - 1).collect();
    // println!("{:?}", dists);
    let dists: Vec<u64> = dists.into_iter().filter(|&x| x > 0).collect();
    if dists.is_empty() {
        println!("{}", 0);
        return;
    }
    let k = *dists.iter().min().unwrap();
    let ans = dists.iter().fold(0u64, |acc, &d| acc + (d + k - 1) / k);
    println!("{}", ans);
}
