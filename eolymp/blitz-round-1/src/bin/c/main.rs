...
fn solve(n: usize, a: Vec<Vec<u32>>) {
    let mut sums = HashSet::new();
    for i in 0..n {
        sums.insert(a[i].iter().sum::<u32>());
    }
    for j in 0..n {
        sums.insert((0..n).map(|i| a[i][j]).sum::<u32>());
    }

    let main_diag = (0..n).map(|i| a[i][i]).collect::<Vec<_>>();
    let sub_diag = (0..n).map(|i| a[i][n - i - 1]).collect::<Vec<_>>();
    for diag in [main_diag, sub_diag] {
        for j in 0..n {
            for i in 0..=j {
                let s = diag[i..=j].iter().sum::<u32>();
                if sums.contains(&s) {
                    println!("YES");
                    return;
                }
