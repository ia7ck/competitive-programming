...
struct Monoid {
  ...
  static Monoid ident() { return Monoid((1LL << 31) - 1); }
...
int main() {
  ...
  scanf("%d %d", &n, &q);
  vector<Monoid> a;
  SegmentTree<Monoid> seg(n, vector<Monoid>(n, Monoid((1LL << 31) - 1)));
  while (q--) {
    int t;
    scanf("%d", &t);
    if (t == 0) {
...
// http://judge.u-aizu.ac.jp/onlinejudge/review.jsp?rid=4017252#1
